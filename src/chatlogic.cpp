...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes - remove as _nodes are now smart pointers and on the stack
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // check if node with this ID exists already (new node is actually a iterator and needs dereferencing)
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            AddAllTokensToElement("ANSWER", tokens, **newNode); //leave this as is, dereference iterator, dereference new smart pointer using overloaded * operator
                        }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root - dereference iterator then get the pointer value from smart pointer
            }
            else
            {
    ...
    // create instance of chatbot
    ChatBot chatBot("../images/chatbot.png"); //declared on the stack

    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    chatBot.SetChatLogicHandle(this);
    // _chatBot = &_chatBot_local; //save address in member
